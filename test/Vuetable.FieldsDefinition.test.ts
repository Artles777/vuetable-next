...
import VuetableFieldHandle from "../src/components/VuetableFieldHandle.vue";

describe("Vuetable - Fields Definition", () => {

  beforeAll( () => {
    // @ts-ignore
    global.console.error = vitest.fn( msg => msg);
    // @ts-ignore
    global.console.warn = vitest.fn( msg => msg);
  });
  ...
  /**
     ...
     */
  it("should use the given dataClass to render field title", context => new Promise(done => {
    const app = createApp({});
  ...
  it("should call the formatter function to format the column value", context => new Promise(done => {
    const app = createApp({});
