...
describe("Vuetable - Methods", () => {
  beforeAll(() => {
    // @ts-ignore
    global.console.warn = vitest.fn((msg: string) => msg);
  });
  ...
  const shallowVuetable = (fields: any[]) => shallowMount(Vuetable, {
    propsData: {
      loadOnStart: false,
      fields
    }
  ...
  describe("normalizeFields", () => {
    ...
    it("copies additional options to fields definition as well", () => {
      const wrapper = shallowVuetable([
        ...
        { name: "description", foo: "bar", formatter: () => "bee" }
      ]);
      ...
      expect(typeof(fields[1].formatter)).toBe("function");
      expect(fields[1].formatter()).toEqual("bee");
    });
  });
  ...
  describe("checkIfRowIdentifierExists", () => {
    ...
    it("returns true when row identifier is valid", (context: TaskContext) => new Promise(done => {
      const app = createApp({});
  ...
  describe("setData", () => {
    ...
    it("emits loading and loaded events when argument is Object", (context: TaskContext) => new Promise((done) => {
      const app = createApp({});
      const wrapper = shallowVuetable(["code"]);

      wrapper.vm.setData({ pagination, data });
  ...
  describe("getAllQueryParams", () => {
    ...
    const q1 = (sortOrder: string[], currentPage: number, perPage: number) => {
      // return object
    ...
    const mountVuetable = (queryParams: any) => shallowMount(Vuetable, {
      propsData: {
        apiMode: false,
        fields: ["code"],
        queryParams
